...
%  Steve Cox, 1/20/03
%
% solve the passive fiber  with current  injection via the trapezoid rule
%
...
function vselect = g_l_trapcab(g_L, cab,stim,pinc)

Cm = 1;		% micro F / cm^2
                %g_L = 1/15; %0.3;     		% mS / cm^2
R_2 = 0.1448; % 0.034;		% k Ohm cm
dx = cab.dx;
dt = cab.dt;
Nx = cab.Nx; %cab.ell/dx;                % patch length
A = 2*pi*cab.rad*dx;            % patch surface area
x = cab.x_grid; %dx/2:dx:cab.ell-dx/2;       % vector of patch midpoints
...
replambda = repmat(lambda(:),1,Nx);

A = 2*pi*cab.rad*dx;

B = (speye(Nx)+replambda.^2.*S)./reptau;
...
recording_places = [1 round(.75*Nx)];
vselect = zeros(2,Nx);
