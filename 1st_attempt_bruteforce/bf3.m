function [error g_l_output] = bf3(basis_fcn, Ntemp, Trange, bf_mod_no)
% Ryan George
...
G = @(T) 1/60;%aff_params(1)*T +aff_params(2); %KOhms
...
stim = (time>1).*(time<1.5);

stim(end+1)=0; %to avoid an error in the last loop
stim = stim*stim_amp*mfac;


cab = struct('rad',1e-4,'ell',.1,'dx', dx, 'dt', dt, 'x_grid', x_grid, ...
             'Nx', Nx);
stim = struct('t1',1,'t2',2,'amp',6e-4,'loc',0.06,'Tfin',10)

runv = @(test_g_l) g_l_trapcab(test_g_l, cab, stim, -1);
...
switch basis_fcn
    case 'step'
        Ngl_els = ceil(Nx/bf_mod_no);        
    case 'hat'
        Ngl_els = ceil(Nx/bf_mod_no) + 2;
    case 'polyn'
        Ngl_els = bf_mod_no;
end
...
options = optimset('MaxFunEvals', 1e3, 'TolFun', 1e-16, 'Display', 'iter');
...
switch basis_fcn
    case 'step'
        ...
        for n = 1:Ngl_els-1
            ...
            g_l_fin( ((Ngl_els-1)*bf_mod_no+1) : Nx ) = g_l_rec(Ngl_els);
...
switch basis_fcn
    ...
    case 'step'
        Ngl_els = numel(g_l_info);        
        g_l_test = zeros(size(x_grid));
        for n = 1:Ngl_els-1
            g_l_test( ((n-1)*bf_mod_no+1):n*bf_mod_no  ) = g_l_info(n);
        end
        g_l_test((Ngl_els-1)*bf_mod_no+1:numel(x_grid))=g_l_info(Ngl_els);
