...
%               stim.loc = location of current pulse (compartment
%               num)
...
%  stim =
%  struct('t1',t1,'t2',t2,'amp',1e-4*ones(1,s),'loc',loc,'Tfin',Tfin)
%

function [v_soma tq teigs mvec] = scdend(dt,stim,pinc)
...
sp_const =  a/(2*R2)/dx^2;   % lambda^2
r = a/a(3);
Hd = [2*sp_const(1)*ones(1,N(1)) 2*sp_const(2)*ones(1,N(2)) 2*sp_const(3)*ones(1, ...
                                                  N(3)+1)];
Hd(1) = sp_const(1);
Hd(N(1)+1) = sp_const(2);
Hd(N(1)+N(2)+1) =  sp_const*r';
Hd(end) = rho*sp_const(3);%rho*sp_const(3);
Hlen = length(Hd)

Hu = [-sp_const(1)*ones(1,N(1)-1) 0 -sp_const(2)*ones(1,N(2)) -sp_const(3)*ones(1, ...
                                                  N(3))];
Hl = [-sp_const(1)*ones(1,N(1)-1) 0 -sp_const(2)*ones(1,N(2)-1) -r(2)*sp_const(2) ...
      -sp_const(3)*ones(1,N(3))];
Hl(end) = rho*Hl(end); % rho*Hl(end);

H = spdiags( [[Hl 0]' Hd' [0 Hu]'], -1:1, Hlen, Hlen);

H(N(1)+N(2)+1,N(1)) = -r(1)*sp_const(1);
H(N(1),N(1)+N(2)+1) = -sp_const(1);

I = speye(Hlen);

%Bb = I+(I+H)*(dt/tau/2); 
Bb = I+(spdiags(gL,0,NN+1,NN+1) +H)*(dt/Cm/2); 
[L,U] = lu(Bb); 

[mvec mval] = eig(full(Bb));
tq = [mvec(1,:)' mvec(N(1)+1,:)' mvec(end,:)'];
teigs = diag(mval);
