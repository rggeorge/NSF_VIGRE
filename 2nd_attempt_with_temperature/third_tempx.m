...
g_0 = 1/15; %mS/cm^2
g_l = x_grid + g_0; %must be a vector of size(x_grid)
...
for T = 1:Ntemp
    ...
    replambda_sq = repmat(lambda_sq(:)', Nx, 1);
    ...
    for n = 2:Nt
        force_piece(1) = dt*(g_l(1)*E_L(T) + stim(n)/C); %stim should be scaled
        v(:,n,T) = U \ ( L \ (v(:,n-1,T) + force_piece));%U \ (L \ (v(:,n-1,T) + force_piece));
    end
