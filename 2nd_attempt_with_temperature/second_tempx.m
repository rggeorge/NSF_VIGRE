...
for T = 1:Ntemp
    %backward euler
    D =  -dt*S*Gvec(T)/C + diag(1 + dt*g_l/C);
    v(:,1, T) = E_L(T); %initial conditions
    
    force_piece = dt*g_l*E_L(T);
    [L,U] = lu(D);
    
    lkage = dt*g_l(1)*E_L(T)/C;
    
    for n = 2:Nt
        force_piece(1) = lkage + dt*stim(n)/C; %stim should be scaled
        v(:,n,T) = U \ (L \ (v(:,n-1,T) + force_piece));
    end
